...
export default function App() {
    ...
    function () {
      ...
      var unsub = onSnapshot(
        ...
        function (snap) {
          ...
          var merged = { ...DD, ...d };
          setData(merged);
          var k = Object.keys(merged.markets || {});
          if (k.length && !selM) setSelM(k[0]);
          setLoaded(true);
        },
        function () {
          setLoaded(true);
  ...
  var persist = useCallback(function (n) {
    setData(n);
    save(n);
  }, []);
  function doSignIn() {
  ...
  function addMarket() {
    ...
    var id = gid();
    persist({ ...data, markets: { ...data.markets, [id]: { name: newMkt.trim() } } });
    setSelM(id);
    setNewMkt("");
  }
  function delMarket(id) {
    ...
    setSelM(rem.length ? rem[0] : null);
  }
  function addRep() {
    if (!newRep.trim() || !selM) return;
    persist({
      ...data,
      reps: {
        ...data.reps,
  ...
  function togRep(rid) {
    persist({ ...data, reps: { ...data.reps, [rid]: { ...data.reps[rid], active: !data.reps[rid].active } } });
  }
  function setRepRole(rid, role) {
    persist({ ...data, reps: { ...data.reps, [rid]: { ...data.reps[rid], role: role } } });
  }
  ...
  function initKI(mId, dt) {
    ...
    reps.forEach(function (p) {
      ...
      if (rep.role === "closer") {
        ...
        inp[id] = ex
          ? { doorsKnocked: ex.doorsKnocked, convosHad: ex.convosHad || "", setsSet: ex.setsSet, apptsRan: ex.apptsRan, closes: ex.closes }
  ...
  useEffect(
    function () {
      if (selM && loaded) initKI(selM, entryDate);
    },
  ...
  function saveKPIs() {
    ...
    Object.entries(kpiIn).forEach(function (p) {
      var rid = p[0],
        v = p[1],
        entry = { repId: rid, date: entryDate, notes: kpiNotes[rid] || "" };
      Object.keys(v).forEach(function (k) {
        entry[k] = parseInt(v[k]) || 0;
      });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F2F2F7" }}>
      ...
      {toast ? (
        <div style={{ position: "fixed", top: 64, right: 14, zIndex: 9999, background: "#1C1C1E", color: "#fff", fontSize: 13, fontWeight: 600, padding: "10px 20px", borderRadius: 12, boxShadow: shL }}>
          {toast}
        </div>
      ) : null}
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "12px" }}>
        ...
        {tab === "dashboard" &&
          (!selM ? (
            <Card>
              <p style={{ color: "#8E8E93", textAlign: "center", margin: 0 }}>Select a market.</p>
            </Card>
          ) : (
            <div>
              ...
              {actionN.length === 0 && coachN.length === 0 && (
                <Card>
        ...
        {tab === "enter" &&
          (!selM ? (
            ...
            <div>
                ...
                return (
                  <Card key={rid}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                      <span style={{ fontWeight: 700, fontSize: 16 }}>{rep.name}</span>
                      <RoleBadge role={rep.role} />
                    </div>
              ...
              {curReps.length > 0 && (
                <Btn onClick={saveKPIs} style={{ padding: "12px 36px", fontSize: 15, borderRadius: 12 }}>
                  Save
                </Btn>
              )}
            </div>
        ...
        {tab === "knockerboard" && (
          <div>
            ...
            <p style={{ fontSize: 12, color: "#8E8E93", margin: "0 0 12px 0" }}>
              {"All markets · Closers included when knocking · " + numDays + " day" + (numDays > 1 ? "s" : "")}
            </p>
            ...
            <Podium
              items={knockerLB.slice(0, 3).map(function (r) {
                return {
                  name: r.name,
                  sub: r.market + (r.role === "closer" ? " · Closer" : ""),
            ...
            {knockerLB.length > 3 && (
              <Card style={{ padding: 0, overflow: "auto", borderRadius: 14 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr>
                      {["#", "Rep", "Market", "", "Doors", "Convos", "Sets", "D2C%"].map(function (h) {
                        return <th key={h} style={{ ...ths, textAlign: h === "Rep" || h === "Market" || h === "" ? "left" : "center" }}>{h}</th>;
                      })}
                    </tr>
                  </thead>
                  <tbody>
                    {knockerLB.slice(3).map(function (r, i) {
                      return (
        ...
        {tab === "closerboard" && (
          <div>
            ...
            <p style={{ fontSize: 12, color: "#8E8E93", margin: "0 0 12px 0" }}>
              {"All markets · " + numDays + " day" + (numDays > 1 ? "s" : "")}
            </p>
            <div style={{ display: "flex", gap: 4, marginBottom: 14, flexWrap: "wrap" }}>
            ...
            <Podium
              items={closerLB.slice(0, 3).map(function (r) {
                ...
                return { name: r.name, sub: r.market, val: vm[clSort], metric: { closes: "Closes", closeRate: "Close%", selfGens: "Mo SG", hours: "Hours" }[clSort] };
              })}
            />
            <Card style={{ padding: 0, overflow: "auto", borderRadius: 14 }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead>
                  <tr>
                    {["#", "Rep", "Market", "Appts", "Closed", "Close%", "CAD%", "Hrs", "Hrs/Day", "Mo SG"].map(function (h) {
                      return <th key={h} style={{ ...ths, textAlign: h === "Rep" || h === "Market" ? "left" : "center" }}>{h}</th>;
                    })}
                  </tr>
                </thead>
                <tbody>
                  ...
                  {closerLB.length === 0 && (
                    <tr>
                      <td colSpan={10} style={{ padding: 20, textAlign: "center", color: "#C7C7CC" }}>
                        No closer data.
                      </td>
                    </tr>
                  )}
                  ...
                  <Card key={rid} bc={log.length > 0 ? ns.color : undefined}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: log.length > 0 ? 10 : 0, flexWrap: "wrap", gap: 6 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <span style={{ fontWeight: 700, fontSize: 15 }}>{rep.name}</span>
                        <RoleBadge role={rep.role} />
          ...
          <div>
            ...
            <Card>
              ...
              {mIds.map(function (mId) {
                var kc = Object.values(data.reps).filter(function (r) { return r.marketId === mId && r.active !== false && r.role !== "closer"; }).length;
                var cc = Object.values(data.reps).filter(function (r) { return r.marketId === mId && r.active !== false && r.role === "closer"; }).length;
                return (
                  <div key={mId} style={{ padding: "10px 0", borderBottom: "1px solid #F2F2F7", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {selM && (
              <Card>
                ...
                {Object.entries(data.reps)
                  .filter(function (p) { return p[1].marketId === selM; })
                  ...
                  .map(function (p) {
                    ...
                    return (
                      <div key={rid} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 0", borderBottom: "1px solid #F2F2F7", flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 600, fontSize: 14, opacity: rep.active !== false ? 1 : 0.4 }}>{rep.name}</span>
                        <RoleBadge role={rep.role} />
                        {tl ? <Badge text={tl} /> : null}
                        {rep.active === false ? <Badge text="Inactive" /> : null}
            ...
            <Card style={{ background: "#FFF0EF", border: "1px solid #FFD4D1" }}>
              ...
              <Btn v="danger" onClick={function () { if (!confirm("DELETE ALL?")) return; persist(DD); setSelM(null); flash("Cleared"); }} style={{ borderRadius: 10 }}>
                Reset
              </Btn>
            </Card>
          </div>
